...
const mangapill = async (title) => {
    const mangapill = new extensions_1.MANGA.MangaPill();
    console.log(`[+] Getting MangaPill mappings for ${chalk_1.default.cyan(title)}`);
    return await mangapill.search(title).then((resp) => {
        ...
        console.log(chalk_1.default.red(`[-] Failed to get mappings for ${title} on MangaPill`));
        return undefined;
    });
};
